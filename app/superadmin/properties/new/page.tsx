...
import { Shield, Upload, ArrowLeft, Save, CheckCircle, AlertCircle, LogOut, Plus, X, Play } from 'lucide-react';
...
export default function NewPropertySuperAdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.title) missingFields.push('Título do Anúncio');
      if (!formData.description) missingFields.push('Descrição');
      if (!formData.propertyType) missingFields.push('Tipo de Imóvel');
      if (!formData.address) missingFields.push('Endereço');
      if (!formData.city) missingFields.push('Cidade');
      if (!formData.state) missingFields.push('Estado');
      if (!formData.bedrooms) missingFields.push('Quartos');
      if (!formData.bathrooms) missingFields.push('Banheiros');
      if (!formData.area) missingFields.push('Área');
      if (!formData.maxGuests) missingFields.push('Número de Hóspedes');
      if (!formData.dailyPrice) missingFields.push('Preço da Diária');
      
      if (missingFields.length > 0) {
        setErrorMessage(`Campos obrigatórios não preenchidos: ${missingFields.join(', ')}`);
        return;
      }

      if (formData.availableForSale && !formData.salePrice) {
        setErrorMessage('Campo obrigatório não preenchido: Preço de Venda (quando disponível para venda)');
        return;
      }

      if (imageUrls.length === 0) {
        setErrorMessage('Campo obrigatório não preenchido: Pelo menos uma imagem deve ser adicionada');
        return;
      }
      ...
      const propertyData = {
        ...
        status: formData.publishToShowcase ? 'published' : 'pending',
        createdAt: new Date().toISOString(),
        publishedAt: formData.publishToShowcase ? new Date().toISOString() : null,
        ownerName: 'Super Admin',
        ownerEmail: user.email,
        featured: formData.publishToShowcase && Math.random() > 0.7, // 30% chance of being featured if published
        source: 'SuperAdmin',
      };

      // Save to super admin properties
      // Save to published properties if publishing to showcase
      if (formData.publishToShowcase) {
        ...
        // Save to pending properties if not publishing to showcase
      ...
      setSuccessMessage(`Imóvel cadastrado com sucesso${formData.publishToShowcase ? ' e publicado na vitrine!' : '!'}`);
      setTimeout(() => {
        router.push('/superadmin');
      }, 2000);
    } catch (error) {
      ...
      <header className="border-b border-border bg-card">
        <div className="flex h-16 items-center justify-between px-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">Cadastrar Novo Imóvel</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center space-x-2 p-4 bg-muted/30 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Se ativado, o imóvel será publicado automaticamente na página de imóveis e na home do site
                    </p>
                  </div>
                </div>
              </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Adicione múltiplas imagens do imóvel</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-4">
              ...
              <div className="pt-6 border-t border-border">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Adicione um vídeo para mostrar melhor o imóvel. Formatos aceitos: MP4, WebM, OGV.
                </p>
