...
export default function EditFunnelPage() {
  ...
  const loadFunnel = () => {
    ...
    if (foundFunnel) {
      ...
      setFunnelData({
        name: foundFunnel.name,
        description: foundFunnel.description || '',
      });
      setStages(foundFunnel.stages || []);
    }
  };
  ...
  const removeStage = (stageId: string) => {
    ...
    const leadsInStage = leads.filter((lead: any) => lead.stageId === stageId);
    
    if (leadsInStage.length > 0) {
      setError(`Não é possível excluir esta etapa pois há ${leadsInStage.length} lead(s) vinculado(s).`);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedFunnels = existingFunnels.map((f: any) => 
        f.id === funnel.id ? updatedFunnel : f
      );
      ...
      setError('Erro ao atualizar funil. Tente novamente.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <header className="border-b border-border bg-card">
        <div className="flex h-16 items-center justify-between px-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground">Editar Funil</h1>
              <p className="text-xs text-muted-foreground">Atualizar funil de vendas</p>
            </div>
          </div>
        </div>
      </header>
